...
public class ErrorsWhileCopyingHandlerActivity extends AppCompatActivity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /// load generic layout
        setContentView(com.telkomsigma.telkomstorage.R.layout.generic_explanation);

        /// customize text message
        TextView textView = findViewById(com.telkomsigma.telkomstorage.R.id.message);
        String appName = getString(com.telkomsigma.telkomstorage.R.string.app_name);
        String message = String.format(getString(com.telkomsigma.telkomstorage.R.string.sync_foreign_files_forgotten_explanation),
                appName, appName, appName, appName, mAccount.name);
        textView.setText(message);
        textView.setMovementMethod(new ScrollingMovementMethod());

        /// load the list of local and remote files that failed
        ListView listView = findViewById(com.telkomsigma.telkomstorage.R.id.list);
        ...
        /// customize buttons
        Button cancelBtn = findViewById(com.telkomsigma.telkomstorage.R.id.cancel);
        Button okBtn = findViewById(com.telkomsigma.telkomstorage.R.id.ok);

        okBtn.setText(com.telkomsigma.telkomstorage.R.string.foreign_files_move);
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (view != null) {
                ...
                if (localPath != null) {
                    ...
                    if (text1 != null) {
                        text1.setText(String.format(getString(com.telkomsigma.telkomstorage.R.string.foreign_files_local_text), localPath));
                    }
                }
                if (mRemotePaths != null && mRemotePaths.size() > 0 && position >= 0 &&
                    ...
                    if (text2 != null && remotePath != null) {
                        text2.setText(String.format(getString(com.telkomsigma.telkomstorage.R.string.foreign_files_remote_text), remotePath));
                    }
                }
            }
            return view;
    ...
    private class MoveFilesTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected void onPreExecute() {
            ...
            mCurrentDialog = IndeterminateProgressDialog.newInstance(com.telkomsigma.telkomstorage.R.string.wait_a_moment, false);
            mCurrentDialog.show(getSupportFragmentManager(), WAIT_DIALOG_TAG);
            findViewById(com.telkomsigma.telkomstorage.R.id.ok).setEnabled(false);
        }
        ...
        protected void onPostExecute(Boolean result) {
            ...
            mCurrentDialog.dismiss();
            mCurrentDialog = null;
            findViewById(com.telkomsigma.telkomstorage.R.id.ok).setEnabled(true);

            if (result) {
                // nothing else to do in this activity
                Toast.makeText(ErrorsWhileCopyingHandlerActivity.this,
                        getString(com.telkomsigma.telkomstorage.R.string.foreign_files_success), Toast.LENGTH_LONG)
                        .show();
                finish();

            } else {
                Snackbar.make(findViewById(android.R.id.content), com.telkomsigma.telkomstorage.R.string.foreign_files_fail, Snackbar.LENGTH_LONG)
                        .show();
            }
        }
    }

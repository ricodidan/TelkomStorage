...
import com.telkomsigma.telkomstorage.ui.dialog.parcel.SyncedFolderParcelable;
import com.telkomsigma.telkomstorage.utils.DisplayUtils;
import com.telkomsigma.telkomstorage.utils.ThemeUtils;

import java.io.File;

import static com.telkomsigma.telkomstorage.datamodel.SyncedFolderDisplayItem.UNPERSISTED_ID;
...
public class SyncedFolderPreferencesDialogFragment extends DialogFragment {
    ...
    public static SyncedFolderPreferencesDialogFragment newInstance(SyncedFolderDisplayItem syncedFolder, int section) {
        ...
        dialogFragment.setStyle(STYLE_NORMAL, com.telkomsigma.telkomstorage.R.style.Theme_ownCloud_Dialog);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mUploadBehaviorItemStrings = getResources().getTextArray(com.telkomsigma.telkomstorage.R.array.pref_behaviour_entries);
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        mView = inflater.inflate(com.telkomsigma.telkomstorage.R.layout.synced_folders_settings_layout, container, false);
    ...
    private void setupDialogElements(View view) {
        ...
        if (mSyncedFolder.getType().getId() > MediaFolderType.CUSTOM.getId()) {
            ...
            view.findViewById(com.telkomsigma.telkomstorage.R.id.local_folder_container).setVisibility(View.GONE);
            view.findViewById(com.telkomsigma.telkomstorage.R.id.delete).setVisibility(View.GONE);
        } else if (mSyncedFolder.getId() <= UNPERSISTED_ID) {
            // Hide delete/enabled for unpersisted custom folders
            view.findViewById(com.telkomsigma.telkomstorage.R.id.delete).setVisibility(View.GONE);
            view.findViewById(com.telkomsigma.telkomstorage.R.id.sync_enabled).setVisibility(View.GONE);

            // auto set custom folder to enabled
            mSyncedFolder.setEnabled(true);

            // switch text to create headline
            ((TextView) view.findViewById(com.telkomsigma.telkomstorage.R.id.synced_folders_settings_title))
                    .setText(com.telkomsigma.telkomstorage.R.string.autoupload_create_new_custom_folder);

            // disable save button
            view.findViewById(com.telkomsigma.telkomstorage.R.id.save).setEnabled(false);
        } else {
            view.findViewById(com.telkomsigma.telkomstorage.R.id.local_folder_container).setVisibility(View.GONE);
        }

        // find/saves UI elements
        mEnabledSwitch = view.findViewById(com.telkomsigma.telkomstorage.R.id.sync_enabled);
        ThemeUtils.tintSwitch(mEnabledSwitch, accentColor);

        mLocalFolderPath = view.findViewById(com.telkomsigma.telkomstorage.R.id.synced_folders_settings_local_folder_path);

        mLocalFolderSummary = view.findViewById(com.telkomsigma.telkomstorage.R.id.local_folder_summary);
        mRemoteFolderSummary = view.findViewById(com.telkomsigma.telkomstorage.R.id.remote_folder_summary);

        mUploadOnWifiCheckbox = view.findViewById(com.telkomsigma.telkomstorage.R.id.setting_instant_upload_on_wifi_checkbox);
        ThemeUtils.tintCheckbox(mUploadOnWifiCheckbox, accentColor);

        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1) {
            view.findViewById(com.telkomsigma.telkomstorage.R.id.setting_instant_upload_on_charging_container).setVisibility(View.GONE);
        } else {
            view.findViewById(com.telkomsigma.telkomstorage.R.id.setting_instant_upload_on_charging_container).setVisibility(View.VISIBLE);

            mUploadOnChargingCheckbox = view.findViewById(
                    com.telkomsigma.telkomstorage.R.id.setting_instant_upload_on_charging_checkbox);
            ThemeUtils.tintCheckbox(mUploadOnChargingCheckbox, accentColor);
        }

        mUploadUseSubfoldersCheckbox = view.findViewById(
                com.telkomsigma.telkomstorage.R.id.setting_instant_upload_path_use_subfolders_checkbox);
        ThemeUtils.tintCheckbox(mUploadUseSubfoldersCheckbox, accentColor);

        mUploadBehaviorSummary = view.findViewById(com.telkomsigma.telkomstorage.R.id.setting_instant_behaviour_summary);

        mCancel = view.findViewById(com.telkomsigma.telkomstorage.R.id.cancel);
        mCancel.setTextColor(accentColor);

        mSave = view.findViewById(com.telkomsigma.telkomstorage.R.id.save);
        ...
        if (mSyncedFolder.getLocalPath() != null && mSyncedFolder.getLocalPath().length() > 0) {
            mLocalFolderPath.setText(
                    DisplayUtils.createTextWithSpan(
                            String.format(
                                    getString(com.telkomsigma.telkomstorage.R.string.synced_folders_preferences_folder_path),
                                    mSyncedFolder.getLocalPath()),
                            mSyncedFolder.getFolderName(),
                            new StyleSpan(Typeface.BOLD)));
            mLocalFolderSummary.setText(mSyncedFolder.getLocalPath());
        } else {
            mLocalFolderSummary.setText(com.telkomsigma.telkomstorage.R.string.choose_local_folder);
        }

        if (mSyncedFolder.getLocalPath() != null && mSyncedFolder.getLocalPath().length() > 0) {
            mRemoteFolderSummary.setText(mSyncedFolder.getRemotePath());
        } else {
            mRemoteFolderSummary.setText(com.telkomsigma.telkomstorage.R.string.choose_remote_folder);
        }
    ...
    private void checkAndUpdateSaveButtonState() {
        if (mSyncedFolder.getLocalPath() != null && mSyncedFolder.getRemotePath() != null) {
            mView.findViewById(com.telkomsigma.telkomstorage.R.id.save).setEnabled(true);
        } else {
            mView.findViewById(com.telkomsigma.telkomstorage.R.id.save).setEnabled(false);
        }
    }
    ...
    private void setupListeners(View view) {
        ...
        view.findViewById(com.telkomsigma.telkomstorage.R.id.delete).setOnClickListener(new OnSyncedFolderDeleteClickListener());

        view.findViewById(com.telkomsigma.telkomstorage.R.id.setting_instant_upload_on_wifi_container).setOnClickListener(
                new OnClickListener() {
                    @Override
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            ...
            view.findViewById(com.telkomsigma.telkomstorage.R.id.setting_instant_upload_on_charging_container).setOnClickListener(
                    new OnClickListener() {
                        @Override
                        public void onClick(View v) {
        ...
        view.findViewById(com.telkomsigma.telkomstorage.R.id.setting_instant_upload_path_use_subfolders_container).setOnClickListener(
                new OnClickListener() {
                    @Override
                    public void onClick(View v) {
        ...
        view.findViewById(com.telkomsigma.telkomstorage.R.id.remote_folder_container).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        view.findViewById(com.telkomsigma.telkomstorage.R.id.local_folder_container).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        view.findViewById(com.telkomsigma.telkomstorage.R.id.sync_enabled).setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                setEnabled(!mSyncedFolder.getEnabled());
            }
        ...
        view.findViewById(com.telkomsigma.telkomstorage.R.id.setting_instant_behaviour_container).setOnClickListener(
                new OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        showBehaviourDialog();
    ...
    private void showBehaviourDialog() {
        ...
        builder.setTitle(ThemeUtils.getColoredTitle(
                getResources().getString(com.telkomsigma.telkomstorage.R.string.prefs_instant_behaviour_dialogTitle),
                ThemeUtils.primaryAccentColor()))
                .setSingleChoiceItems(getResources().getTextArray(com.telkomsigma.telkomstorage.R.array.pref_behaviour_entries),
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        mSyncedFolder.setUploadAction(
                                                getResources().getTextArray(
                                                        com.telkomsigma.telkomstorage.R.array.pref_behaviour_entryValues)[which].toString());

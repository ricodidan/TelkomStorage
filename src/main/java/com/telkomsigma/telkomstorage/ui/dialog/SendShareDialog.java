...
import com.telkomsigma.telkomstorage.ui.adapter.SendButtonAdapter;
import com.telkomsigma.telkomstorage.ui.components.SendButtonData;
...
public class SendShareDialog extends BottomSheetDialogFragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        view = inflater.inflate(com.telkomsigma.telkomstorage.R.layout.send_share_fragment, container, false);

        // Share with people
        TextView sharePeopleText = view.findViewById(com.telkomsigma.telkomstorage.R.id.share_people_button);
        sharePeopleText.setOnClickListener(v -> shareFile(file));

        ImageView sharePeopleImageView = view.findViewById(com.telkomsigma.telkomstorage.R.id.share_people_icon);
        themeShareButtonImage(sharePeopleImageView);
        sharePeopleImageView.setOnClickListener(v -> shareFile(file));

        // Share via link button
        TextView shareLinkText = view.findViewById(com.telkomsigma.telkomstorage.R.id.share_link_button);
        shareLinkText.setOnClickListener(v -> shareFile(file));

        ImageView shareLinkImageView = view.findViewById(com.telkomsigma.telkomstorage.R.id.share_link_icon);
        ...
        if (getContext().getString(com.telkomsigma.telkomstorage.R.string.send_files_to_other_apps).equalsIgnoreCase("off")) {
            sharePeopleText.setVisibility(View.GONE);
        }

        SendButtonAdapter.ClickListener clickListener = setupSendButtonClickListener(sendIntent);

        RecyclerView sendButtonsView = view.findViewById(com.telkomsigma.telkomstorage.R.id.send_button_recycler_view);
    ...
    private void showResharingNotAllowedSnackbar() {
        Snackbar snackbar = Snackbar.make(view, com.telkomsigma.telkomstorage.R.string.resharing_is_not_allowed, Snackbar.LENGTH_LONG);
        ...
        return sendButtonDataData -> {
            ...
            if (MimeTypeUtil.isImage(file) && !file.isDown()) {
                ...
                if (file.isDown()) {
                    ...
                    getActivity().startActivity(Intent.createChooser(sendIntent, getString(com.telkomsigma.telkomstorage.R.string.send)));
